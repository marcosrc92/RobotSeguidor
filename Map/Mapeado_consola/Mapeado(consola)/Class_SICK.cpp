...
void Class_SICK::trama() {
	...
	unsigned char msg_trama[] = { 0x02, 0x00, 0x02, 0x00, 0x30, 0x01, 0x31, 0x18 };
	...
	fprintf(stderr, "Sending bytes...");
	//capturar error en la escritura
	if (!WriteFile(hSerial, msg_trama, 8, &bytes_written, NULL))
	{
		fprintf(stderr, "Error\n");
		CloseHandle(hSerial);
		return;
	...
	fprintf(stderr, "Waiting response...");
	//capturar el error en la lectura
	if (!ReadFile(hSerial, &response, 9, &bytes_read, NULL))
	{
		fprintf(stderr, "Error\n");
	...
	int i = 0;
	while (1) {
		ReadFile(hSerial, &response, 9, &bytes_read, NULL);
		//datagrama de respuesta al reset
		cout << i++ << endl;
		cout << "Datagrama de respuesta: " << response << endl;
		cout << "bytes leidos: " << bytes_read << endl;
	}
